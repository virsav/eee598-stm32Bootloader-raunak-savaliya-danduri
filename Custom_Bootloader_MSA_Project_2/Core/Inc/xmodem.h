...
#define X_EOT ((uint8_t)0x04u)  /*  Transmission End. */
#define X_ACK ((uint8_t)0x06u)  /* Acknowledge. */
#define X_NAK ((uint8_t)0x15u)  /* Not Acknowledge. */
#define X_CAN ((uint8_t)0x18u)  /* Cancel communication. */
#define X_C   ((uint8_t)0x43u)  /* ASCII "C" to make the host aware, we want to use CRC16. */

/* Define the status constants for XMODEM protocol */
typedef enum {
  X_OK            = 0x00u, /*Successful operation. */
